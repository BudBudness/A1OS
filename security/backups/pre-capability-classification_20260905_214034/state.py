...
class A1OS:
    ...
    async def _capability_build_change(
        ...
        **kwargs,
        ...
        if not isinstance(_human_approval, HumanApprovalMarker):
            raise RuntimeError("Human approval marker required")

        if _human_approval.capability != "build_change":
            raise RuntimeError("Approval capability mismatch")

        if _human_approval.command != request:
            raise RuntimeError("Approval command mismatch")
    ...
    async def _capability_deploy_application(
        ...
        **kwargs,
        ...
        if _human_approval.capability != "deploy_application":
            raise RuntimeError("Approval capability mismatch")

        if _human_approval.command.strip().lower() != (
            f"deploy application {product}".strip().lower()
        ):
            raise RuntimeError("Approval command mismatch")
