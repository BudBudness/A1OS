...
async def approve(request: ApprovalRequest):
    ...
    # ------------------------------------------------------------
    # APPROVED TERMINAL EXECUTOR
    # ------------------------------------------------------------
    # Terminal commands are explicitly approved by the human through
    # the token flow above, then delegated through ExecutionBroker.
    # There is intentionally NO arbitrary-shell fallback.
    if command_lower.startswith(("run ", "execute ", "terminal ", "termux ")):
        terminal_parts = command_text.split(maxsplit=1)
        terminal_command = (
            terminal_parts[1].strip()
        ...
        if not terminal_command:
            raise HTTPException(
                status_code=400,
                detail="Missing terminal command.",
            )

        result = await _execution_broker.execute(
            terminal_command,
            approved=True,
        )
    ...
    if command_lower in {
        ...
        result = await _execution_broker.execute(
            "python3 tools/a1os_factory/one_click_factory.py "
    ...
    # ------------------------------------------------------------
        ...
        result = await _execution_broker.execute(
            "python3 tools/a1os_factory/real_build_executor/"
            "build_executor_engine.py "
            + shlex.quote(command_text),
            approved=True,
        )
        _record(
            "execution_result",
    ...
    # ------------------------------------------------------------
        ...
        raise HTTPException(
            status_code=400,
            detail="No approved executor is registered for this command.",
        )
