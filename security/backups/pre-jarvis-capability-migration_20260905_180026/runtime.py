    ...
    async def execute(self, task_id: str, payload: Dict[str, Any]):
        ...
        try:
            if system is not None:
                ...
                # SECURITY HARDENING:
                # Generic target -> getattr() -> execute() is prohibited.
                # All externally supplied execution must pass through
                # system.execute() -> CapabilityRegistry -> consequence gate.
                raise RuntimeError(
                    "EXECUTION AUTHORITY VIOLATION: "
                    "unregistered target execution is prohibited; "
                    "all tasks must execute through system.execute()"
                )
