...
class CapabilityRegistry:
    ...
    async def execute(self, name, **kwargs):
        ...
        approval_marker = kwargs.get("_human_approval")

        gate_kwargs = dict(kwargs)
        gate_kwargs["_human_approval"] = approval_marker
        ...
        handler_kwargs.pop("_human_approval", None)
...
class A1OS:
    def __init__(self):
        ...
        self.finance_worker = FinanceWorker(db="data/a1os.db")
    ...
    async def _capability_build_change(
        self,
        request: str = "",
        **kwargs,
    ):
        """Execute the fixed A1OS build executor; never a shell."""
    ...
    async def _capability_deploy_application(
        ...
        **kwargs,
        ...
        """Execute the fixed A1OS deployment plane; never a shell."""
        if not isinstance(product, str) or not product.strip():
            raise RuntimeError(
                "deploy_application requires a product"
            )

        if not isinstance(template_slug, str) or not template_slug.strip():
            raise RuntimeError(
                "deploy_application requires a template_slug"
            )
    ...
    def _register_capabilities(self):
        ...
        self.capabilities.register(
            "finance_worker",
            self._capability_finance_worker,
        )
        self.capabilities.register(
            "build_change",
            self._capability_build_change,
        )
        self.capabilities.register(
            "deploy_application",
            self._capability_deploy_application,
        )



    async def _capability_finance_worker(self, **kwargs):
        event = dict(kwargs)
        ...
        if not isinstance(data, dict):
            raise TypeError("Finance capability data must be a dictionary")

        event["action"] = action
        event["data"] = data

        return await self.finance_worker.execute(event)
    ...
    async def _universal_consequence_gate(
        ...
        read_only_capabilities = {
            # Core control-plane read-only capabilities.
            "health",
            "health_check",
            "diagnostics",
            "capabilities",
            ...
            # Digital-world read-only capabilities.
            "digital_world_model",
            "digital_world_graph",
            "digital_world_query",
            "digital_world_state",
        ...
        if isinstance(approval, HumanApprovalMarker):
            entity_id = approval.entity_id
            target_action = approval.target_action or target_action
    ...
    "deploy_application",
        ...
        if not isinstance(approval, HumanApprovalMarker):
            return {
                "allowed": False,
                "requires_authorization": True,
                "classification": "consequential",
                ...
                "reason": (
                    ...
                    "Autonomous authorization is disabled."
                ),
            }
        ...
        if approval.capability != capability:
            return {
                ...
                "capability": capability,
                "decision": "human_required",
                "reason": "Approval capability mismatch.",
            }
        ...
        # Exact capability/argument binding is enforced at the authority boundary.
        if capability == "build_change":
            requested = kwargs.get("request", "")
            if approval.command != requested:
                return {
                    "allowed": False,
                    "requires_authorization": True,
                    "classification": "consequential",
                    "capability": capability,
                    "decision": "human_required",
                    "reason": "Approval command mismatch.",
                }

        if capability == "deploy_application":
            product = kwargs.get("product", "platform")
            expected = f"deploy application {product}".strip().lower()
            if approval.command.strip().lower() != expected:
                return {
                    "allowed": False,
                    "requires_authorization": True,
                    "classification": "consequential",
        ...
        authorization = {
            ...
            "decision": "human_authorized",
            "confidence": 1.0,
            "reason": "Explicit human approval verified in-process.",
        }
    ...
    async def _capability_sovereignty_policy_learning(
        ...
        """
        Policy-learning subsystem.

        SECURITY INVARIANT:
        This subsystem may classify, evaluate, and learn policy,
        but it MUST NEVER grant execution authority.

        Consequential execution authority can only originate from
        a separately verified human approval and must subsequently
        pass the universal consequence gate.
        """
        ...
        if operation == "authorize":
            return {
                ...
                "decision": "human_required",
                "reason": (
                    "Policy-learning cannot grant consequential "
                    "execution authority. Explicit human authorization "
                    "is required."
                ),
                "capability": capability,
                "entity_id": entity_id,
    ...
    async def _capability_digital_world_graph(
        ...
        **kwargs,
        ...
        if operation in {"read", "get", "current"}:
            runtime = getattr(self, "runtime", None)
            runtime_path = None
            if runtime is not None:
                runtime_path = getattr(runtime, "runtime_path", None)
            ...
            rows = db.execute("""
                SELECT id, entity_type, state, metadata, updated_at
                FROM entities
                ORDER BY id
            """).fetchall()
            ...
            return {
                "status": "digital_world_state_complete",
                "entities": [
                    {
                        "id": row["id"],
    ...
    async def _capability_digital_world_state(
        ...
        **kwargs,
        ...
        if operation in {"read", "get", "current", "summary"}:
            rows = db.execute("""
