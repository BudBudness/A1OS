...
def run() -> dict:
    ...
    requirements = {"capabilities": [], "roles": ["owner"], "features": ["release-evidence"]}
    engine_results = []
    with tempfile.TemporaryDirectory(prefix="a1os-factory-") as work:
        ...
        build = execute("release-evidence", products, evidence)
        result = {
            "status": "PASS" if validation["status"] == "healthy" and build["status"] == "built" and len(engine_results) == 37 else "FAIL",
