...
CORE_LAUNCH="$HOME/A1OS_RESTORED/ops/a1os-core-launch.sh"
API_LOG="$HOME/education-os-api-runtime.log"
WEB_LOG="$HOME/education-os-web-runtime.log"

echo "=== STARTING A1OS CORE (127.0.0.1:3011) ==="
"$CORE_LAUNCH" || {
    echo "[FAIL] CORE"
    exit 1
}
...
exec cloudflared tunnel --protocol http2 --config "$HOME/.cloudflared/config.yml" run a1os-prod
