...
export async function login(email,password) {
  ...
  if (!data?.token) throw new Error("Authentication response did not include a session token");
  sessionStorage.setItem("a1os_session_token", data.token);
...
export async function logout() {
  ...
  sessionStorage.removeItem("a1os_session_token");
  principal=null;
  globalThis.__A1OS_PRINCIPAL__=null;
