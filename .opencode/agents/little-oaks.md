...
- A1OS core API: http://127.0.0.1:3011 (`python3 main.py`, binds `127.0.0.1`, in watchdog scope via `ops/a1os-core-launch.sh`; serves `edge.pyongcity.org`/`pos.edge.pyongcity.org`)
...
- `curl -s http://127.0.0.1:3011/v1/health` (core — expected healthy; restart via `ops/a1os-core-launch.sh` if down)
- `curl -s http://127.0.0.1:8080/`
- `curl -s https://little-oaks.pyongcity.org/api/health`

Watchdog: `ops/a1os-production-watchdog.sh` runs hourly via crontab `~/crontab.txt`; it checks core, API, frontend, public tunnel, and DB integrity, auto-restarts core/API/web/tunnel (core via `ops/a1os-core-launch.sh`), and fires a ntfy.sh alert on FAIL/CRITICAL (topic in `~/.a1os/ntfy.topic`).
