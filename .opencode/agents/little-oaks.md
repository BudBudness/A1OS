...
- Frontend + `/api` proxy: http://127.0.0.1:8080 (`~/education-os-web-server.py`, serves `products/education-os/web`)
...
Watchdog: `ops/a1os-production-watchdog.sh` runs hourly via crontab `~/crontab.txt`; it checks core, API, frontend, public tunnel, and DB integrity, and auto-restarts core/API via the canonical launcher.
Backups: `~/backup-little-oaks-education-db.sh` runs daily 01:00 and Sunday 12:00; it writes integrity-checked sqlite `.backup` files to `products/education-os/deployments/little-oaks/backups/` (30-day retention).
...
- Restart services only via their canonical launcher: `run-production.sh` for :3012, `~/education-os-launch.sh` for the full stack.
