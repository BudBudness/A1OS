...
- Core API: `python3 main.py` → uvicorn on :3011 (`core/api.py`). **Decommissioned** — stopped, removed from watchdog scope (idle/unused by the product). `a1ctl` won't respond while it's stopped.
...
- **3011** — A1OS core engine (`python3 main.py`). **Decommissioned** — stopped and removed from the watchdog's check/restart scope (idle/unused by the edu product); edu owns 3012. To bring it back: run `python3 main.py` and re-enable the commented watchdog block.
