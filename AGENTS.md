...
- **3011** — A1OS core engine (`python3 main.py`). Supervised by the watchdog (optional/idle); edu owns 3012.
...
`a1ctl` talks to the core on 3011 (runs `python3 main.py`). The stack is not guaranteed up at any given moment — the watchdog restarts anything it finds down on its hourly run; to bring the full stack up manually, run `education-os-launch.sh`.
