...
const nav=[["Dashboard","dashboard"],["Projects","projects"],["Clients","clients"],["Estimates","estimates"],["Invoices","invoices"],["Payments","payments"],["Tasks","tasks"],["Materials","materials"],["Sites","sites"],["Reports","reports"]];
const empty={projects:[],clients:[],estimates:[],invoices:[],payments:[],tasks:[],documents:[],materials:[],sites:[]};
...
function Form({type,onDone,onCancel}){
 ...
 const fields={
  ...
  estimates:[["title","Estimate title"],["description","Description"],["amount","Amount"],["client_id","Client ID"]],
  ...
  documents:[["name","Document name"],["document_type","Document type"],["url","Document URL"]],materials:[["name","Material"],["unit","Unit"],["quantity","Quantity"],["unit_cost","Unit cost"],["project_id","Project ID"]],sites:[["name","Site"],["location","Location"],["client_id","Client ID"],["status","Status"]]
 }[type]||[];
...
function Dashboard(){
 const [s,setS]=useState({projects:0,clients:0,estimates:0,invoices:0,payments:0,tasks:0});
...
export function App(){
 ...
 const views={
  ...
  estimates:<Resource type="estimates" label="Estimates & Quotes" keyName="estimates" fields={["title","amount","status","client_id"]}/>,
  ...
  reports:<Resource type="reports/summary" label="Financial Reports" keyName="reports" fields={["metric","value"]}/>
 };
 return <div className="app"><header><div><h1>Construction OS</h1><p>Project & Construction Management</p></div><strong>UGX</strong></header><div className="layout"><nav>{nav.map(([l,k])=><button className={active===k?"active":""} onClick={()=>setActive(k)} key={k}>{l}</button>)}</nav><main>{views[active]}<Resource type="materials" label="Materials" fields={fields.materials}/><Resource type="sites" label="Sites" fields={fields.sites}/></main></div></div>;
}
