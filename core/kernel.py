...
class Kernel:
    ...
    def process_input(self, task):
        ...
        if task.get("role") != "admin" and target == "governance": return {"error": "Unauthorized"}
        if target in self.halt_list: return {"error": "Halted"}
        ...
        if not is_valid:
            ...
            result = self.workers[target].process_task(task) # Retry once
